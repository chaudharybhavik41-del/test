...
<body>
  <div class="card">
    <h2>QC Hold</h2>
    <form method="post" action="qc_hold_post.php" onsubmit="return validateForm()">
      ...
      <div class="row">
        ...
        <div class="field">
          <label>QC Bin (destination)</label>
      ...
      <button class="btn primary" type="submit">Hold to QC</button>
    </form>
  </div>
...
function validateForm(){
  ...
  const fromBin = document.getElementById('from_bin_id').value;
  const qcBin = document.getElementById('qc_bin_id').value;
  if (!wh || !qcBin) { alert('Warehouse and QC bin are required'); return false; }
  if (fromBin && qcBin && fromBin === qcBin) { alert('From Bin and QC Bin cannot be same'); return false; }
  return true;
}
