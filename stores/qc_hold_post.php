...
/**
 * QC Hold POST
 * - Creates qc_holds header+lines
 * - Creates a stock_transfers header (TRN) and posts OUT from from_bin -> IN to qc_bin (same warehouse)
 * - Mirrors to stock_ledger and updates valuation (IN at source WA)
 */
...
try {
  ...
  // Collect lines from form arrays or JSON
  $lines = [];
  ...
  $qcNo = NumberingService::next($pdo, 'QCH');

  // qc_holds header
  $pdo->prepare("INSERT INTO qc_holds (qc_no, warehouse_id, from_bin_id, qc_bin_id, remarks, status, created_by, created_at)
                 VALUES (:no,:w,:fb,:qb,:rm,'POSTED',:uid,NOW(6))")
      ->execute([':no'=>$qcNo, ':w'=>$warehouseId, ':fb'=>$fromBinId, ':qb'=>$qcBinId, ':rm'=>$remarks, ':uid'=>$userId]);
  $qcId = (int)$pdo->lastInsertId();

  $insLine = $pdo->prepare("INSERT INTO qc_hold_items (qc_id, line_no, item_id, uom_id, qty, from_bin_id, qc_bin_id, batch_id, remarks)
                            VALUES (:id,:ln,:i,:u,:q,:fb,:qb,:b,:r)");
  ...
  $pdo->prepare("INSERT INTO stock_transfers (trn_no, from_warehouse_id, to_warehouse_id, from_bin_id, to_bin_id, remarks, status, created_by, created_at)
                 VALUES (:no,:w,:w,:fb,:qb,:rm,'POSTED',:uid,NOW(6))")
      ->execute([':no'=>$qcNo, ':w'=>$warehouseId, ':fb'=>$fromBinId, ':qb'=>$qcBinId, ':rm'=>$remarks, ':uid'=>$userId]);
  ...
  foreach ($lines as $ln) {
    ...
    $insLine->execute([':id'=>$qcId, ':ln'=>$lineNo, ':i'=>$itemId, ':u'=>$uomId, ':q'=>$qty, ':fb'=>$fromBinId, ':qb'=>$qcBinId, ':b'=>$batchId, ':r'=>$r]);

    $waQ->execute([':i'=>$itemId, ':w'=>$warehouseId]); $rate=(float)$waQ->fetchColumn(); if ($rate<0) $rate=0.0;

    // OUT from free/bin to QC bin
    $outPayload=['txn_type'=>'TRN','txn_no'=>$qcNo,'txn_date'=>$now,'project_id'=>null,'item_id'=>$itemId,'uom_id'=>$uomId,
      'warehouse_id'=>$warehouseId,'bin_id'=>$fromBinId,'batch_id'=>$batchId,'qty'=>$qty,'unit_cost'=>$rate,
  ...
  $pdo->commit();
  echo json_encode(['ok'=>true,'qc_id'=>$qcId,'qc_no'=>$qcNo]);
