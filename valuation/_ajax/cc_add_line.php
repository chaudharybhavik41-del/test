...
try{ require_login(); require_permission('stores.cc.edit'); $pdo=db();
  $in=json_decode(file_get_contents('php://input'),true)?:$_POST;
  $svc=new \Coupler\CycleCountService($pdo);
  $id=$svc->addLine((int)($in['cc_id']??0),(int)($in['item_id']??0),(float)($in['counted_qty']??0));
  echo json_encode(['ok'=>true,'data'=>['line_id'=>$id]]);
